...
const ProductCard = ({ product, featured, hideTags = false }) => {
  ...
  return (
    <article
      ...
      className="group relative border border-[var(--color-border-soft)] bg-white overflow-hidden transition-all hover-lift h-full flex flex-col cursor-pointer rounded-lg shadow-sm"
    >
      <div className="relative">
        ...
        <div className="absolute inset-x-4 bottom-4 flex items-center justify-between gap-2 opacity-0 translate-y-3 transition-all group-hover:opacity-100 group-hover:translate-y-0">
          ...
          <button
            ...
            className={`h-10 w-10 grid place-items-center rounded-full border transition-all shadow-sm ${favorited ? 'bg-[var(--color-primary)] text-white border-[var(--color-primary)]' : 'bg-white text-[var(--color-primary)] border-[var(--color-border-soft)] hover:border-[var(--color-accent)]'
              }`}
